...
from cairo_lfd.data.vectorization import vectorize_demonstration, get_observation_joint_vector, get_observation_pose_vector
...
def main():
    ...
    rk_processor = RelativeKinematicsProcessor(acclfd.environment.get_item_ids(), acclfd.environment.get_robot_id())
    ic_processor = InContactProcessor(acclfd.environment.get_item_ids(), acclfd.environment.get_robot_id(), .06, .5)
    soi_processor = SphereOfInfluenceProcessor(acclfd.environment.get_item_ids(), acclfd.environment.get_robot_id())
    rp_processor = RelativePositionProcessor(acclfd.environment.get_item_ids(), acclfd.environment.get_robot_id())
    wp_processor = WithinPerimeterProcessor(acclfd.environment.get_item_ids(), acclfd.environment.get_robot_id())
    ...
    recorder = SawyerDemonstrationRecorder(rec_settings, acclfd.environment, processing_pipeline, publish_constraint_validity=True)
    ...
    if args.input_directory is not None:
        ...
        acclfd.build_keyframe_graph(labeled_initial_demos, model_settings.get("bandwidth", .025))
        acclfd.generate_autoconstraints(labeled_initial_demos)
        acclfd.sample_keyframes(model_settings.get("number_of_samples", 50), automate_threshold=True)
    else:
    ...
    study = ACCLfDController(acclfd, recorder, demo_labeler, labeled_initial_demos, output_directory, task_name, subject)
    study.run()
