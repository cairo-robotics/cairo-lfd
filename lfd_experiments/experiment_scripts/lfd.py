...
from cairo_lfd.controllers.study_controllers import LfDController
...
def main():
    ...
    rk_processor = RelativeKinematicsProcessor(lfd.environment.get_item_ids(), lfd.environment.get_robot_id())
    ic_processor = InContactProcessor(lfd.environment.get_item_ids(), lfd.environment.get_robot_id(), .06, .5)
    soi_processor = SphereOfInfluenceProcessor(lfd.environment.get_item_ids(), lfd.environment.get_robot_id())
    rp_processor = RelativePositionProcessor(lfd.environment.get_item_ids(), lfd.environment.get_robot_id())
    wp_processor = WithinPerimeterProcessor(lfd.environment.get_item_ids(), lfd.environment.get_robot_id())
    ...
    recorder = SawyerDemonstrationRecorder(rec_settings, lfd.environment, processing_pipeline, publish_constraint_validity=True)
    ...
    #########################
    study = LfDController(lfd, recorder, demo_labeler, labeled_initial_demos, output_directory, task_name, subject)
    study.run()
