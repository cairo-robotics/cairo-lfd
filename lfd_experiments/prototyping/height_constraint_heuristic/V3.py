...
def plot_results(X, Y_, means, covariances, ax):
	''' Purpose: Plots any given segmentation with ellipsoid gaussian distribution
		then finds upper and lower bound of ellipsoid and samples 5 linearly spaced
		points within that range. It then takes those 5 points and plots them both as
		individual points as well as planes
	'''
	...
	for i, (mean, covar, color) in enumerate(zip(
		...
		ax.scatter3D(x[maxzindouter][maxzindinner],y[maxzindouter][maxzindinner],z[maxzindouter][maxzindinner], color='red')
		ax.scatter3D(x[minzindouter][minzindinner],y[minzindouter][minzindinner],z[minzindouter][minzindinner], color='red')

		# plot upper height as plane
		psize = 0.1*z[maxzindouter][maxzindinner]
		plot_plane(psize,x,y,z,maxzindouter,maxzindinner,ax)

		# plot lower height as plane
		psize = 0.1*z[maxzindouter][maxzindinner]
		plot_plane(psize,x,y,z,minzindouter,minzindinner,ax)	

		# find indices of and plot middle heights as smaller planes
		psize = 0.05*z[maxzindouter][maxzindinner]	
		[zindouter,zindinner] = find_mid_height_index(z,heights[1])
		ax.scatter3D(x[zindouter][zindinner],y[zindouter][zindinner],z[zindouter][zindinner], color='red')
		plot_plane(psize,x,y,z,zindouter,zindinner,ax)
		[zindouter,zindinner] = find_mid_height_index(z,heights[2])
		ax.scatter3D(x[zindouter][zindinner],y[zindouter][zindinner],z[zindouter][zindinner], color='red')
		plot_plane(psize,x,y,z,zindouter,zindinner,ax)
		[zindouter,zindinner] = find_mid_height_index(z,heights[3])
		ax.scatter3D(x[zindouter][zindinner],y[zindouter][zindinner],z[zindouter][zindinner], color='red')	
		plot_plane(psize,x,y,z,zindouter,zindinner,ax)

		#plt.show()

def find_mid_height_index(z,height):
	'''
	Purpose: Attempts to find the nearest point on the ellipsoid that matches a given height
	'''
	least_diff = 100	
	for i in range(len(z)):
		[ind,value] = find_nearest(z[i],height)
		if abs(value-height) < least_diff:
			least_diff = abs(value-height)
			zindouter = i
			zindinner = ind
	return (zindouter,zindinner)

def find_nearest(array, value):
	'''
	Purpose: Given and array and a value, attemps to find nearest point in array to
	specified value
	'''
...
def plot_plane(psize,x,y,z,outerind,innerind,ax):
	'''
	Purpose: Given x, y, z, data structures and outer and inner indices,
	defines a plane using a point and a normal vector, and then plots it
	'''
	...
	try:
		...
		s = np.array([x[outerind-1][innerind-1],y[outerind-1][innerind-1],z[outerind][innerind]])
