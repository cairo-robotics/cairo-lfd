...
from cairo_lfd.constraints.metaconstraint import MetaConstraintGenerator, HeightMetaConstraint
...
def main():
    ...
    # Create height segmentation model
    ...
    height_constraint_generator = MetaConstraintGenerator(height_segment_model, HeightMetaConstraint, height_static_parameters)
    for node in graph.get_keyframe_sequence():
        vectors = []
        for observation in graph.nodes[node]["observations"]:
            vectors.append(vectorize_robot_position(observation))
        graph.nodes[node]["meta_constraints"]["height"] = height_constraint_generator.generate_meta_constraint(vectors)
        for constraint in graph.nodes[node]["meta_constraints"]["height"].constraints:
            print(constraint)
        print
        print
    ...
    for node in graph.get_keyframe_sequence():
        ...
        constraints = [random.choice(graph.nodes[node]["meta_constraints"]["height"].constraints)]
