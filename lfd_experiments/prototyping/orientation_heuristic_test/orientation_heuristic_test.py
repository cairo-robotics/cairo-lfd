...
from cairo_lfd.constraints.heuristics import OrientationHeuristicModel
from cairo_lfd.constraints.metaconstraints import HeightMetaconstraint
from cairo_lfd.constraints.metaconstraint_assignment import MetaconstraintAssigner, OrientationMetaconstraintBuilder
...
def main():
    ...
    parser.add_argument(
        '-t', '--threshold', type=int, default=-100, metavar='THRESHOLD',
    ...
    for cluster_id in clusters.keys():
        ...
        graph.nodes[cluster_id]["metaconstraints"] = []
    ...
    # Create orientation segmentation and heuristic model
    orientation_demo_vectorizor = partial(vectorize_demonstration, vectorizors=[vectorize_robot_orientation])
    orientation_vectors = np.array(map(orientation_demo_vectorizor, demonstrations))
    # stack all observation vectors
    X = np.vstack(orientation_vectors)
    orientation_segment_model = BayesianGMMSegmentModel(X, n_components=10)
    orientation_heuristic_model = OrientationHeuristicModel(orientation_segment_model)
    orientation_heuristic_model.fit()
    orientation_static_parameters = {
        "item_id": 1
    }
    orientation_metaconstraint_builder = OrientationMetaconstraintBuilder(orientation_heuristic_model, orientation_static_parameters)
    metaconstraint_assigner = MetaconstraintAssigner(environment, graph, [orientation_metaconstraint_builder])
    ...
    for node in graph.get_keyframe_sequence():
        ...
        constraints = [meta.constraints[0] for meta in graph.nodes[node]["metaconstraints"]]
