...
def plot_results(X, Y_, means, covariances, fig=plt.figure()):
    ...
    for i, (mean, covar, color) in enumerate(zip(
        ...
        if not np.any(Y_ == i):
            continue
        ax.scatter(X[Y_ == i, 0], X[Y_ == i, 1],
                   X[Y_ == i, 2], s=.8, color=color)
...
class VBGMMSegmentation():
    ...
    def build_model(self, data):
        ...
        plot_results(X, self.vbgmm.predict(
            X), self.vbgmm.means_, self.vbgmm.covariances_)
        plt.show()
    ...
    def segment(self, data):
        ...
        for index in range(len(startindices)):
            if startindices[index] == endindices[index]:
                segments.append(
                    data[startindices[index]:endindices[index] + 1])
            else:
        ...
        for demoind in range(len(allsegments)):
            for segind in range(len(allsegments[demoind])):
                ...
                if len(segment) > 1:
                    ...
                    X = np.array([e for sl in demo for e in sl])
                    if segind == 0:
                        fig = plot_results(X, vbgmm.predict(
                            X), vbgmm.means_, vbgmm.covariances_)
                    else:
                        plot_results(X, vbgmm.predict(
                            X), vbgmm.means_, vbgmm.covariances_, fig)
        plt.show()
        # {
        #   1: [segment1for1, segment2for1, segment3for1],
        #   2: [segment1for2, segment2for2, segment3for2]
...
def main():
    ...
    # Plot segments
    demo_segmenter.plot_segments(
        allsegments, segobjects, vbgmm_segmenter.vbgmm)
