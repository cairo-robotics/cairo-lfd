...
class VariationalGMM():
    ...
    def __init__(self, n_components=3, max_iter=200, tolerance=1e-6, alpha_prior=None, beta_prior=1, dof=None,
        ...
        # number of iterations to run iterative update of varational inference
        self.max_iter = max_iter
        self.tolerance = tolerance  # Log-likelihood tolerance for terminating EM
        # Dirichlet parameter for prior of weights. (1, K)
        ...
        # Initial covariances of mixands.
    ...
    def _initialize_parameters(self, X):
        ...
        self.alpha_k = np.full([self.n_components, ],
                               self.alpha_prior)  # dirichlet parameters
        ...
        self.beta_k = np.full(self.n_components, self.beta_prior)
        ...
        self.W_lb = np.zeros(
            [self.n_components, self.n_features_,
             self.n_features_])  # Collects inverse Wishart scaling matricies for use in lower bound
        self.W_prior = self.wishart_matrix_prior if self.wishart_matrix_prior is not None else np.atleast_2d(
            np.cov(X.T))
        # Inverse of initial wishart component
        self.W_prior_inv = np.linalg.inv(self.W_prior)
    ...
    def e_step(self, X):
        ...
        # In the variational E step, the ultimate goal is to calculate the responsibilities.
    ...
    def fit(self, X):
        ...
        if abs(new_lb - prior_lb) > self.tolerance:
            print(
                "Algorithm maximum iterations inadequate to achieve convergence according to given tolerance.")
        self.log_resp, self.resp = self.e_step(X)
        self.fitted = True
        return self
    ...
    def _estimate_gaussian_statistics(self, X, resp):
        ...
        for k in range(0, self.n_components):
            x_cen = X - x_bar_k[k]
            S_k[k] = np.dot(resp[:, k] * x_cen.T, x_cen) / \
                N_k[k]  # Bishop equation 10.53
    ...
    def _update_wishart_prior_parameters(self, N_k):
        # from Bishop 10.63 and according to sci-kit learn, it shouldn't have the +1
        self.nu_k = self.dof + N_k
    ...
    def _estimate_wishart_matrix(self, N_k, x_bar_k, S_k):
        ...
        self.precisions_cholesky_ = compute_precisions_cholesky(
            self.covariances_)

    def _update_expected_log_pi(self):
        self.log_pi = digamma(self.alpha_k) - \
            digamma(np.sum(self.alpha_k))  # from Bishop 10.66
    ...
    def _update_expected_log_lambda(self):
        for k in range(0, self.n_components):
            ...
            self.log_lambda[k] = digamma_sum + self.n_features_ * np.log(2) + np.log(
                np.linalg.det(self.precisions_cholesky_[k]))  # from Bishop 10.65
    ...
    def _calculate_lower_bound(self, N_k, x_bar_k, S_k):
        ...
        for k in range(0, self.n_components):
            ...
            # see Bishop 10.74
            log_pml2 = log_pml2 + \
                self.nu_k[k] * np.trace(np.dot(self.W_prior_inv, self.W_lb[k]))
